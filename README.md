...
`assets/models/fishdex-source.blend` preserves the original Blender scenes. [The source archive](assets/blender/README.md) retains version 3 and adds editable version 4 scenes with packed textures and a verified inventory. [The current model review](docs/model-review/v4/index.html) shows all 31 assets with side and angled Blender renders. `scripts/modeling/build_refined.py` generates the current revision. `assets/models/*.glb` are runtime exports; `assets/specimens/` holds rendered previews. `npm run models:bundle` embeds GLBs and the Three.js viewer for offline use. No external model hosting or CDN is used. Third-party renderer licenses remain in `src/generated/viewer.ts` and `node_modules/three/LICENSE`.
