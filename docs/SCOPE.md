...
Private iPhone field journal for Dad, Mason, and Nolan. Initial install target is the family's fishing trip on September 7, 2026; Expo EAS internal distribution first, TestFlight for friends later. No public App Store release or cloud account service.
...
Type-check, iOS bundle, storage/backup validation tests, visual review, native device smoke test when available, EAS build and install link. A cloud build is not a successful on-device test. Document unvalidated identification and model accuracy honestly.

## Design

Palette: forest #183F35, reed #B7CA9D, paper #F4F2E8, lake #527D8A, ink #223B34, brass #D8AE55. Georgia titles evoke field notes; system sans keeps controls legible. One large specimen stage, quiet ruled journal rows, thumb-friendly camera action. Avoid dense game HUDs. Natural textures and colors match the explicitly requested journal direction.
